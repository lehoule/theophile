...
if (!input) throw new Error('Usage: node scripts/import-wordpress.mjs path/to/wordpress.xml');
const parser = new XMLParser({ ignoreAttributes: false, removeNSPrefix: false, isArray: (name) => name === 'item' || name === 'category' || name === 'wp:postmeta' });
...
const pages = items.filter((item) => item['wp:post_type'] === 'page' && item['wp:status'] === 'publish');
const turndown = new TurndownService({ headingStyle: 'atx', bulletListMarker: '-' });
turndown.addRule('wordpressComments', { filter: (node) => node.nodeType === 8, replacement: () => '' });
const value = (entry) => typeof entry === 'object' && entry !== null && '#text' in entry ? entry['#text'] : String(entry ?? '');
const yaml = (value) => JSON.stringify(value);
const dateFor = (item) => {
  const raw = value(item['wp:post_date_gmt']) || value(item['wp:post_date']) || value(item.pubDate);
  const candidate = raw && !raw.startsWith('0000-00-00') ? new Date(raw) : new Date(value(item.pubDate));
  return Number.isFinite(candidate.valueOf()) ? candidate : new Date(value(item.pubDate));
};
...
function writeEntry(item, directory) {
  ...
  const frontmatter = [
    '---', `title: ${yaml(title)}`, `slug: ${yaml(slug)}`, `publishedAt: ${date.toISOString()}`,
    Number.isFinite(modified.valueOf()) ? `updatedAt: ${modified.toISOString()}` : '',
    excerpt ? `excerpt: ${yaml(excerpt)}` : '', `author: ${yaml(value(item['dc:creator']) || 'Théophile')}`,
    `categories: ${yaml(categories(item))}`, `tags: ${yaml(tags(item))}`,
  ...
  return { id, title, slug, date: date.toISOString(), categories: categories(item), tags: tags(item), shortcodes, mediaReferences };
}
const importedPosts = posts.map((item) => writeEntry(item, 'src/content/posts'));
const importedPages = pages.map((item) => writeEntry(item, 'src/content/pages'));
fs.mkdirSync('migration', { recursive: true });
fs.writeFileSync('migration/content-report.json', JSON.stringify({
  source: input,
  posts: importedPosts.length,
  pages: importedPages.length,
  categories: [...new Set(importedPosts.flatMap((post) => post.categories))].length,
  tags: [...new Set(importedPosts.flatMap((post) => post.tags))].length,
  unknownShortcodes: [...new Set([...importedPosts, ...importedPages].flatMap((entry) => entry.shortcodes))],
  mediaReferences: [...new Set([...importedPosts, ...importedPages].flatMap((entry) => entry.mediaReferences))],
}, null, 2));
console.log(`Imported ${importedPosts.length} posts and ${importedPages.length} pages. Review migration/content-report.json before publishing.`);
